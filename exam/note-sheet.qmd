...
2. *expected classification error $\mu = E[\text{error}_n]$ not a function of the training data?*: $\mu_n$ is data-independent, it is a function only of the classifiction rule.
...
4. *Is a consistent classification rule always better than a non-consistent one and why?*: No. non-consist. is better when $n$ is small because consist. class. tned to be complex.
...
9. *The penalty term in an SVM?*: Small C includes outlier (soft margin and less overfitting); big C ignores outliers (hard margin and more overfitting).

10. *How many points does the minimal nonlinearly-separable problem in 2 dimensions have?*:  4, XOR data set.

11. *Cover-Hart Thm.*: The expected error of the NN classification rule satisfies $\epsilon_{NN} = \lim_{n\to \infty} E[\epsilon_n] = E[2\eta(X)(1-\eta(X))]$. $\epsilon_{NN} \leq 2\epsilon^{*}(1-\epsilon^{*})\leq 2\epsilon^{*}$. "The error of the nearest-neighbot classifier with a large sample size cannot be worse than two times the Bayes error." $\epsilon_{NN}\geq \epsilon_{3NN}\geq \epsilon^{*}$
...
- **Ch. 3**: *Classification rule vs. classifier*: output classifiers ; class lables
