...
- Both classifiers fail to distinguish `pearlite`

``` {python}
## Multiclassifier
clf = OneVsOneClassifier(svm.SVC(kernel="rbf", C=1., gamma="auto").fit(features_train["block5_pool"],\
          ltrain["label"].to_numpy(int)))
clf.fit(features_train["block5_pool"],\
          ltrain["label"].to_numpy(int))
...
- The multiclass one-to-one classifier only select `spheroidite` or `network` for unseen two categories.
