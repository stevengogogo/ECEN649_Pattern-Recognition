...
\begin{align}
    \epsilon^{*} &= P(Y=0)\epsilon^{0}[\psi^*] + P(Y=1)\epsilon^1 [\psi^*]\\
                 &= \sum_{\{x|P(Y=1)p(x|Y=1)>P(Y=0)p(x|Y=0)\}} P(Y=0)p(x|Y=0) \\ 
    &+ \sum_{\{x|P(Y=1)p(x|Y=1)\leq P(Y=0)p(x|Y=0)\}} P(Y=1)p(x|Y=1) 
\end{align}
...
class GaussianBayesClassifier:
    ...
    def plot2D(self, psi_annotates=[[0.3,0.3], [0.7,0.7]]):
        ...
        # Create girds
        xlist = np.linspace(-3.5,3.5,200)
        ylist = np.linspace(-3.5,3.5,200)
