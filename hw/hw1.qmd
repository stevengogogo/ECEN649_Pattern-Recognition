...
$$E[\eta(X)] = \sum_{x\in R^d} P(Y=1|X=x)p(x) = P(Y=1)$$
...
\begin{align}
    ...
    \begin{bmatrix}
        1 + (l_k - 2)\sigma_i & -\sigma_i & \cdots & -\sigma_i\\ 
        -\sigma_i & 1 + (l_k - 2)\sigma_i &  \cdots & -\sigma_i\\ 
        \vdots & \vdots & \ddots & \vdots\\ 
        -\sigma_i & -\sigma_i & \cdots & 1 + (l_k - 2)\sigma_i\\ 
    \end{bmatrix}\\ 
    &= 
    \begin{bmatrix}
        \frac{1 + (l_k - 2)\sigma_i}{\sigma_{i}^{2}(1-\sigma_i)(1+(l_k - 1)\sigma_i)}& \frac{-\sigma_i}{\sigma_{i}^{2}(1-\sigma_i)(1+(l_k - 1)\sigma_i)} & \cdots & \frac{-\sigma_i}{\sigma_{i}^{2}(1-\sigma_i)(1+(l_k - 1)\sigma_i)}\\
        \frac{-\sigma_i}{\sigma_{i}^{2}(1-\sigma_i)(1+(l_k - 1)\sigma_i)} &  \frac{1 + (l_k - 2)\sigma_i}{\sigma_{i}^{2}(1-\sigma_i)(1+(l_k - 1)\sigma_i)} & \cdots & \frac{-\sigma_i}{\sigma_{i}^{2}(1-\sigma_i)(1+(l_k - 1)\sigma_i)}\\
        \vdots & \vdots & \ddots & \vdots\\ 
        \frac{-\sigma_i}{\sigma_{i}^{2}(1-\sigma_i)(1+(l_k - 1)\sigma_i)} & \cdots & \frac{-\sigma_i}{\sigma_{i}^{2}(1-\sigma_i)(1+(l_k - 1)\sigma_i)} & \frac{1 + (l_k - 2)\sigma_i}{\sigma_{i}^{2}(1-\sigma_i)(1+(l_k - 1)\sigma_i)}\label{eq:inv}\\
    \end{bmatrix}
\end{align}
...
\begin{align}
    \epsilon^{*}_{L} = \Phi(\frac{-1}{2}\delta) &= \frac{-1}{2}\sqrt{(\mu_1 - \mu_0)^T \Sigma^{-1}(\mu_1 - \mu_0)}\\ 
    &= \frac{-1}{2}\sqrt{%
        \underbrace{%
    \begin{bmatrix}
        1 & \cdots & 1
...
class GeneralMultiGaussian:
    ...
    def cluster_cov_inv(self):
        sig = self.sig
        l = self.l
        scale = 1/((sig**2)*(1-sig)*(1+(l-1)*sig))
        diag_val = 1 + (l-2)*sig 
        covInv = np.ones((l,l)) * (-1*sig)
    ...
    def cov_inv(self):
        d = self.d; l = self.l
        subMs = [self.subCovInv for i in range(0,int(d/l))]
        return block_diag(*subMs)
    ...
    def bayesError(self):
        ...
        return -0.5 * np.sqrt(mud.T @ CovInv @ mud)



sigs = np.linspace(0.01 ,3 , 10)
d = 20
l = 4
rhos = [0, 0.25, 0.5, 0.75]


gm = GeneralMultiGaussian(4, 2, sigs[0], rhos[0])
