...
def findroot(g0,g1, bound=[2.5,7.5]):
    f = lambda x: g1.pdf(x) - g0.pdf(x)
    return opt.brentq(f, *bound)

g0 = Gauss(1,3,2)
g1 = Gauss(3,4,18)
dec_x = findroot(g0,g1)
...
ax.axvline(x=dec_x, color='red', label="Bayes decision boundary")
ax.set_xlabel("$x$")
ax.set_ylabel("$PDF$")
ax.annotate("$\psi^{*}(x)=1$ if $\\eta(x)\\approx$ " + str(round(dec_x,2)), (7.5,0.2))
ax.annotate("$\psi^{*}(x)=0$", (-5,0.2))
