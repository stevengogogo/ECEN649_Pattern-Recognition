...
def get_an(m0,m1):
    return hat_mu(m1) - hat_mu(m0)

def get_bn(m0,m1):
    hmu0 = hat_mu(m0)
    hmu1 = hat_mu(m1)
    return (hmu1 - hmu0)*(hmu1+hmu0).T/2

def epsilon(m0, m1, p0=0.5):
    p1 = 1-p0
    hmu0 = hat_mu(m0)
    hmu1 = hat_mu(m1)
    an = get_an(m0, m1)
...
r  = np.ones(k) 
n0 = binom(n,1-c)
#X0 = gen_class(n0,m0,k,l,s0,r)
#n1 = n-n0
#X1 = gen_class(n1,m1,k,l,s1,r)
# np.mean(X1, axis=0)

#print(epsilon(np.matrix(X0), np.matrix(X1)))
...
for n in ns:
    for d in ds:
        ens = np.zeros(M)
        for i in range(0,M):
            X0 = gen_class(n, m0, k, l, s, r)
            X1 = gen_class(n, m1, k, l, s, r)
            ens[i] =  epsilon(X0, X1)[0]
        res[n][d] = np.mean(ens)
```
